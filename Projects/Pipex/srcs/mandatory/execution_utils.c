	...
	while (true)
	{
		...
		if (user_input == NULL)
			terminate_pipex_with_msg(pipex_info, "Error in get_next_line input",
									ERR_IN_GNL, FD2);
		if (ft_strncmp(user_input, pipex_info->heredoc_delim, ft_strlen(pipex_info->heredoc_delim)) == 0)
		{
			free(user_input);
			break;
		}
...
char	*get_executable_cmd(t_pipex *pipex_info, char *system_path, char *cmd)
{
	...
	if (executable_cmd == NULL || !executable_cmd[0])
		terminate_pipex_with_msg(pipex_info, "Failed in mallocing strjoin of child process.\n",
								ERR_FAIL_STRJOIN_CMDS, FD2);
	free(unusable_cmd);
...
void	execute_cmd(t_pipex *pipex_info, int i)
{
	...
	if(pipex_info->commands->cmd_pers_validation == PER_NOT_X_R_NOK)
		terminate_pipex_with_msg(pipex_info, "Failed :( unfortunately, the command isn't executable/readable.\n",
								ERR_CMD_NOT_XR, FD2);
	if (pipex_info->commands[i].cmd_pers_validation == PERMISSIONS_OK)
	{
		if (access(pipex_info->commands[i].full_path, F_OK | X_OK | R_OK) == 0)
			execve(pipex_info->commands[i].full_path, pipex_info->commands[i].cmd_args, pipex_info->env.raw_env);
	}
	k = 0;
	while (pipex_info->env.system_paths[k] != NULL)
	{
		// ft_printf("cmd arg 0 : %s\n", pipex_info->commands[i].cmd_args[0]);
		executable_cmd = get_executable_cmd(pipex_info, pipex_info->env.system_paths[k], pipex_info->commands[i].cmd_args[0]);
		if (access(executable_cmd, F_OK | X_OK | R_OK) == 0)
			execve(executable_cmd, pipex_info->commands[i].cmd_args, pipex_info->env.raw_env);
		free(executable_cmd);
		k++;
	}
...
int	handle_input_file(t_pipex *pipex_info, int i, int j)
{
	...
	if (pipex_info->pipes.childs_prcs_ids[i] == -1)
	{
		terminate_pipex_with_msg(pipex_info, "Error: Failed in forking input child.\n", errno, i);
